...
// The deleted-message row is the reviewed feature port from upstream PR #2.
// Its hook implementation is kept separate so this existing settings screen
// remains the single owner of table sections and switch state.
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if (indexPath.section == 0) { // GHOST MOODE
		...
		if (indexPath.row == 0) {
			...
			cell.textLabel.text = TGLoc(@"SHOW_DELETED_MESSAGES_TITLE");
			cell.detailTextLabel.text = TGLoc(@"SHOW_DELETED_MESSAGES_SUBTITLE");
		}
...
- (void)switchChanged:(UISwitch *)sender {
	...
	if (switchKey) {
		...
		if ([switchKey isEqualToString:kTGExtraShowDeletedMessages]) {
			TGExtraSetDeletedMessagesEnabled(sender.isOn);
		}
	}
}
    ...
    switch (indexPath.section) {
		...
		case 2:
		    switch (indexPath.row) {
		        ...
		        case 2: return kTGExtraShowDeletedMessages;
		        default: return nil;
		    }
        case 3:
            switch (indexPath.row) {
